...
class map_editor():
	...
	def make_menu(self):
		if self.custom_menu:
			...
			blips.add_command(label="blip named",     command=lambda: self.blip_outlines('blip'))
			blips.add_command(label="blip",           command=lambda: self.blip_outlines('blip_encounter'))
			blips.add_command(label="blip encounter", command=lambda: self.blip_outlines('blip_special'))
			blips.add_command(label="blip special",   command=lambda: self.blip_outlines('new_blip'))
			option_menu.add_cascade(label="outlines", menu=blips)

			blips = tkinter.Menu(option_menu, tearoff=0)
			blips.add_command(label="blip place",     command=lambda: self.blip_visibility('blip_place'))
			blips.add_command(label="blip named",     command=lambda: self.blip_visibility('blip'))
			blips.add_command(label="blip",           command=lambda: self.blip_visibility('blip_encounter'))
			blips.add_command(label="blip encounter", command=lambda: self.blip_visibility('blip_special'))
			blips.add_command(label="blip special",   command=lambda: self.blip_visibility('new_blip'))
			option_menu.add_cascade(label="hide blips", menu=blips)
